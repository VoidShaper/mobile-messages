  ...
  trait Setup {
    ...
    val expectedGetMessagesResponse =
      Json.parse(s"""
         ...
         |    "readTimeUrl": "${encrypted(messageId1)}",
         |    "sentInError": false
         |  },
         |  {
         |    "id": "$messageId2",
         |    "subject": "message subject",
         |    "validFrom": "${validFromDate.toString()}",
         |    "readTimeUrl": "${encrypted(messageId2)}",
    ...
    def encrypted(value: String) = {
      Base64.encode(
        CryptoWithKeysFromConfig(baseConfigKey = "queryParameter.encryption").
          encrypt(PlainText(value)).value.getBytes
      )
    }
